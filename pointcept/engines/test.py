...
class TesterBase:
    def __init__(self, cfg, model=None, test_loader=None, verbose=False) -> None:
        ...
        voxel_gpu_index = faiss.index_cpu_to_gpu(res, 0, voxel_index)
        faiss.contrib.torch_utils.handle_torch_Index(voxel_gpu_index)
        self.voxel_index = voxel_gpu_index
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if os.path.isfile(pred_save_path):
                ...
                for i in range(len(fragment_list)):
                    fragment_batch_size = 32
                    s_i, e_i = i * fragment_batch_size, min(
                        (i + 1) * fragment_batch_size, len(fragment_list) - 1
                    )
                    if s_i >= e_i:
                        break
                    ...
                    with torch.no_grad():
                        pred_part = self.model(input_dict, self.voxel_index)["seg_logits"]  # (n, k)
                    ...
                    logger.info(
                        "Test: {}/{}-{data_name}, Batch: {batch_idx}/{batch_num}".format(
                            ...
                            data_name=data_name,
                            batch_idx=i,
                            batch_num=len(fragment_list) // fragment_batch_size,
                        )
                    )
                pred = pred.max(1)[1].data.cpu().numpy()
                print(np.max(pred, axis=0))
