...
class Collect(object):
    def __init__(self, keys, offset_keys_dict=None, **kwargs):
        ...
        if offset_keys_dict is None:
            if "point_coord" in keys:
                offset_keys_dict = dict(offset="coord", point_offset="point_coord")
            else:
...
class GridSample(object):
    def __init__(
        ...
        return_point_coord=False,
        point_feat_keys=("coord", "color", "normal"),
        return_point_patch=False,
        ...
        self.return_point_coord = return_point_coord
        self.point_feat_keys = point_feat_keys
        self.return_point_patch = return_point_patch
        self.point_patch_feat_key = point_patch_feat_key
        self.side = side
        self.max_point_num = max_point_num

    def __call__(self, data_dict):
        # print(type(data_dict))
        # print("grid sample", data_dict.keys())
        ...
        if self.return_point_coord:
            data_dict["point_coord"] = data_dict["coord"]
            point_feat = []
            for point_feat_key in self.point_feat_keys:
                point_feat.append(data_dict[point_feat_key])
            data_dict["point_feat"] = np.concatenate(point_feat, axis=-1)
            data_dict["count"] = count
        if self.return_point_patch:
            center = np.random.randint(0, grid_coord.shape[0]-1)
            center = grid_coord[center]
            max = center + np.floor(self.side / self.grid_size)
            min = center - np.floor(self.side / self.grid_size)
            bool_min = np.all(grid_coord >= min,1)
            bool_max = np.all(grid_coord <= max,1)
            bool_idx = bool_min & bool_max
            data_dict["point_patch_coord"] = data_dict["coord"][bool_idx]
            data_dict["point_patch_label"] = data_dict["segment"][bool_idx]
            point_feat = []
            for point_feat_key in self.point_patch_feat_key:
                point_feat.append(data_dict[point_feat_key])
            data_dict["point_patch_feat"] = np.concatenate(point_feat, axis=-1)
            data_dict["point_patch_feat"] = data_dict["point_patch_feat"][bool_idx]
            if data_dict["point_patch_coord"].shape[0] > self.max_point_num:
                data_dict["point_patch_coord"] = data_dict["point_patch_coord"][:self.max_point_num]
                data_dict["point_patch_feat"] = data_dict["point_patch_feat"][:self.max_point_num]
                data_dict["point_patch_label"] = data_dict["point_patch_label"][:self.max_point_num]
            data_dict["point_patch_offset"] = data_dict["point_patch_coord"].shape[0]
...
class SphereCrop(object):
    ...
    def __call__(self, data_dict):
            ...
            if data_dict["coord"].shape[0] > point_max:
                ...
                while idx_uni.size != data_dict["index"].shape[0]:
                    ...
                    if "grid_coord" in data_dict.keys():
                        data_crop_dict["grid_coord"] = data_dict["grid_coord"][
                            idx_crop
                        ]
                    if "normal" in data_dict.keys():
                    ...
                    # if "point_coord" in data_dict.keys():
                    #     mask = np.zeros(data_dict["count"].shape[0], dtype=np.int)
                    #     mask[idx_crop] = 1
                    #     mask = np.repeat_interleave(mask, data_dict["count"])
                    #     data_crop_dict["point_coord"] = data_dict["point_coord"][mask]
                    #     data_crop_dict["point_feat"] = data_dict["point_feat"][mask]
                    #     data_crop_dict["point_offset"] = data_dict["point_coord"].shape[0]
        ...
        # mode is "random" or "center"
            ...
            #     mask[idx_crop] = 1
            #     mask = np.repeat_interleave(mask, data_dict["count"])
            #     data_dict["point_coord"] = data_dict["point_coord"][mask]
            #     data_dict["point_feat"] = data_dict["point_feat"][mask]
            #     data_dict["point_offset"] = data_dict["point_coord"].shape[0]
            #     data_dict["count"] = data_dict["count"][idx_crop]
        return data_dict

@TRANSFORMS.register_module()
class DropPoint(object):
    def __init__(self, point_max=80000):
        self.point_max = point_max
    def __call__(self, data_dict):
        if "point_coord" in data_dict.keys():
            if data_dict["point_coord"].shape[0] > self.point_max:
                shuffle_index = np.arange(data_dict["point_coord"].shape[0])
                np.random.shuffle(shuffle_index)
                shuffle_index = shuffle_index[:self.point_max]
                data_dict["point_coord"] = data_dict["point_coord"][shuffle_index]
...
class ShufflePoint(object):
    def __call__(self, data_dict):
        ...
        # if "point_coord" in data_dict.keys():
        #     coord_list = np.split(data_dict["point_coord"], np.cumsum(data_dict["count"])[0:-1])
        #     feat_list = np.split(data_dict["point_feat"], np.cumsum(data_dict["count"])[0:-1])
        #     coord_list = coord_list[shuffle_index]
        #     feat_list = feat_list[shuffle_index]
        #     data_dict["point_coord"] = np.concatenate(coord_list, axis=0)
        #     data_dict["point_feat"] = np.concatenate(feat_list, axis=0)
        #     data_dict["count"] = data_dict["count"][shuffle_index]
...
class Compose(object):
    ...
    def __call__(self, data_dict):
        for t in self.transforms:
            ...
            # print(t, data_dict.keys())
        return data_dict

@TRANSFORMS.register_module()
class IntensityExpand(object):
    def __init__(self, channels=3):
        self.channels = channels
    def __call__(self, data_dict):
        if "strength" in data_dict.keys():
            data_dict["strength"] = np.repeat(data_dict["strength"], self.channels, axis=-1)
        # print(data_dict.keys())
        return data_dict

@TRANSFORMS.register_module()
class PointPatch(object):
    def __init__(self, grid_size=0.8, point_max=80000, feat_keys=("coord", "color")):
        self.grid_size = grid_size
        self.point_max = point_max
        self.feat_keys = feat_keys
    def __call__(self, data_dict):   
        coord = data_dict["coord"]
        label = data_dict["segment"]

        
        feat = []
        for key in self.feat_keys:
            feat.append(data_dict[key])
        feat = np.concatenate(feat, axis=-1)
        if coord.shape[0] > 400000:
            coord = coord[:400000]
            label = label[:400000]
            feat = feat[:400000]
        center = np.random.randint(0, coord.shape[0]-1)
        min = coord[center] - self.grid_size/2
        max = coord[center] + self.grid_size/2
        bool_min = np.all(coord>min, 1)
        bool_max = np.all(coord<max, 1)
        bool = bool_min & bool_max

        point_patch_coord = coord[bool]
        point_patch_label = label[bool]
        point_patch_feat = feat[bool]
        # if point_patch_coord.shape[0] != 0 :
        if point_patch_coord.shape[0] > self.point_max:
            point_patch_coord = point_patch_coord[:self.point_max]
            point_patch_label = point_patch_label[:self.point_max]
            point_patch_feat = point_patch_feat[:self.point_max]
        data_dict["point_patch_coord"] = point_patch_coord
        data_dict["point_patch_label"] = point_patch_label
        data_dict["point_patch_feat"] = point_patch_feat
        data_dict["point_patch_offset"] = point_patch_coord.shape[0]

        # print("end point patch, time: ", time_b-time_a)
        # print("end point patch", data_dict.keys())
        return data_dict
            # self.grid_size = self.grid_size*2
            # count =  count+1
        # if coord.shape[0] > self.point_max:
        #     data_dict["point_patch_coord"] = coord[:self.point_max]
        #     data_dict["point_patch_label"] = label[:self.point_max]
        #     data_dict["point_patch_feat"] = feat[:self.point_max]
        #     data_dict["point_patch_offset"] = data_dict["point_patch_coord"].shape[0]
        # else:
        #     data_dict["point_patch_coord"] = coord
        #     data_dict["point_patch_label"] = label
        #     data_dict["point_patch_feat"] = feat
        #     data_dict["point_patch_offset"] = data_dict["point_patch_coord"].shape[0]

            
@TRANSFORMS.register_module()
class PointSphere(object):
    def __init__(self, radius=0.8, point_max=80000, feat_keys=("coord", "color")):
        self.radius = radius
        self.point_max = point_max
        self.feat_keys = feat_keys
    def __call__(self, data_dict):
        ...
        while True:
            center = np.random.randint(0, coord.shape[0]-1)
            r = np.linalg.norm(coord-coord[center])
            idx = r < self.radius
            point_patch_coord = coord[idx]
            if point_patch_coord.shape[0] != 0 and point_patch_coord.shape[0] > 512:
                point_patch_label = label[idx]
                point_patch_feat = feat[idx]
                if point_patch_coord.shape[0] > self.point_max:
                    point_patch_coord = point_patch_coord[:self.point_max]
                    point_patch_label = point_patch_label[:self.point_max]
                    point_patch_feat = point_patch_feat[:self.point_max]
