...
class PointPromptTraining(nn.Module):
    ...
    def forward(self, data_dict):
        device = data_dict["segment"].device
        condition = data_dict["condition"][0]
        segment_label_map = torch.as_tensor(self.valid_index[self.conditions.index(condition)]).to(device)
        segment_label_map = torch.cat((segment_label_map, torch.tensor([-1]).to(device)), dim=0)
        # print("data_dict_condition: ", data_dict["condition"])
        # print("segment_label_map: ", segment_label_map)
        # print("data_dict_segment: ", torch.max(data_dict["segment"])) 
        # print("data_dict_segment: ", torch.min(data_dict["segment"]))
        data_dict["segment"] = segment_label_map[data_dict["segment"]].to(data_dict["segment"].device)
        # print("data_dict_segment: ", torch.max(data_dict["segment"]))
        
        # print("data_dict_size: ", data_dict["coord"].shape)
